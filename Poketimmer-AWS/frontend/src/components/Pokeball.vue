...
<template>
  <button
    @click="$emit('click')"
    :class="['pokeball-container', { completed }]"
    :style="{ '--size': size + 'px' }"
    ...
    <svg
      ...
      <!-- Pokeball SVG mejorado -->
      <g class="pokeball-group" :class="{ completada: completed }">
        <!-- Parte superior (roja/verde) -->
        <path
          :class="['pokeball-top', { completada: completed }]"
        ...
        <!-- Parte inferior (blanca/gris) -->
        <path
          :class="['pokeball-bottom', { completada: completed }]"
...
.pokeball-container:hover {
  background: rgba(79, 196, 207, 0.15);
  transform: scale(1.1);
}

.pokeball-container.completed {
  animation: bounce-complete 0.6s ease-in-out;
}
...
.pokeball-container.completed .pokeball-svg {
  filter: drop-shadow(0 6px 12px rgba(81, 201, 30, 0.4));
}

/* Estilos para estado pendiente (rojo y blanco) */
.pokeball-top {
  fill: #d32f2f;
...
/* Estilos para estado completado (verde) */
.pokeball-top.completada {
  fill: #51c91e;
}

.pokeball-bottom.completada {
  fill: #3fa517;
}

.pokeball-center.completada {
  fill: #2d6b0f;
}

/* Animación de cierre */
.pokeball-group.completada {
  animation: pokeball-close 0.6s ease-in-out;
}
